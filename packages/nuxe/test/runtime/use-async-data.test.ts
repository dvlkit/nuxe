...
describe('useAsyncData (server)', () => {
    ...
    it('populates data from default() before the handler resolves', async () => {
        ...
        const handler = vi.fn(() => new Promise<number>((r) => {
            resolveHandler = r
        }))
...
describe('useAsyncData (deduplication)', () => {
    beforeEach(() => {
        vi.stubGlobal('window', {})
        setHydratedPayload({})
    })
    ...
    it('runs the handler once when two consumers share the same key + handler', async () => {
        const app = createSSRApp(defineComponent({render: () => null}))
        const ssrContext: NuxeSSRContext = {
            url: '/',
            request: new Request('http://localhost/'),
        ...
        await app.runWithContext(async () => {
            const a = useAsyncData<{ items: unknown[] }>('shared', handler)
            const b = useAsyncData<{ items: unknown[] }>('shared', handler)
            await ssrContext.awaitAll()
            await nextTick()
    ...
    it('warns when a second consumer registers the same key with a different handler', async () => {
        ...
        await app.runWithContext(async () => {
            useAsyncData('same', async () => 'first')
            useAsyncData('same', async () => 'second')
            await nextTick()

            expect(warnSpy).toHaveBeenCalled()
            const message = warnSpy.mock.calls.map((c) => String(c[0])).join('\n')
            expect(message).toMatch(/same/)
            expect(message).toMatch(/handler/)
        })

        warnSpy.mockRestore()
    })

    it('concurrent refresh() calls converge to a single fetch', async () => {
        let invocations = 0
        const handler = vi.fn(async () => {
            invocations += 1
        ...
        createNuxeApp({
            ...
            router: {} as any,
            config: {public: {}},
            ssrContext: undefined as unknown as NuxeSSRContext,
            state: createNuxeState(),
        })

        let result1!: { data: Ref<string | null>; refresh: () => Promise<void> }
        let result2!: { data: Ref<string | null>; refresh: () => Promise<void> }
        app.runWithContext(() => {
            result1 = useAsyncData<string>('shared', handler) as ReturnType<typeof useAsyncData<string>>
            result2 = useAsyncData<string>('shared', handler) as ReturnType<typeof useAsyncData<string>>
        })

        await vi.waitFor(() => {
            expect(result1.data.value).toMatch(/^v-\d+$/)
        })
        invocations = 0
        handler.mockClear()

        await Promise.all([result1.refresh(), result2.refresh()])

        expect(handler).toHaveBeenCalledTimes(1)
        expect(result1.data.value).toBe(result2.data.value)
    })

    it('reuses the same entry across consumer mount/unmount cycles', async () => {
        const app = createSSRApp(defineComponent({ render: () => null }))
        createNuxeApp({
            vueApp: app,
            router: {} as any,
        ...
        let first!: { data: Ref<string | null> }
        app.runWithContext(() => {
            inner.run(() => {
                first = useAsyncData<string>('lifecycle', handler) as ReturnType<typeof useAsyncData<string>>
            })
        })
        await vi.waitFor(() => {
            expect(first.data.value).toBe('ok')
        })
        const callsAfterMount = handler.mock.calls.length

        inner.stop()

        const re = effectScope()
        let second!: { data: Ref<string | null> }
        app.runWithContext(() => {
            re.run(() => {
                second = useAsyncData<string>('lifecycle', handler) as ReturnType<typeof useAsyncData<string>>
            })
        })

        expect(handler).toHaveBeenCalledTimes(callsAfterMount)
